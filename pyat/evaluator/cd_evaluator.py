...
class CognitiveDiagnosisEvaluator(object):
    ...
    def evaluate(
        ...
        user_params = {
            k: nn.Parameter(
                torch.zeros(len(dataset), *p.detach().shape).to(self.device),
                requires_grad=True,
            )
        ...
        for j in tqdm(range(len(dataset)), disable=silent):
            ...
            train_items = train_rand.sample(
                candidate_items, min(train_size, len(candidate_items))
            )
            session["selected"] = train_items
            session["unselected"] = list(
                set(all_items).difference(set(session["selected"]))
            )
            ...
            self.model.init_user_params(
                params={k: p[j].data for k, p in user_params.items()}
            )
            ...
            if isinstance(self.model, MetaModel):
                all_models[user_no] = {
                    k: v.detach() for k, v in self.model.adaptive_hyper_model.items()
                }
            else:
                all_models[user_no] = {
                    k: v.detach() for k, v in self.model.state_dict().items()
                }
