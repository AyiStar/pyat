...
def show_dataset_info(train_dataobj: Dict, test_dataobj: Dict, logger: logging.Logger):
...
train_triplets = pd.read_csv(f"./train_triplets.csv", encoding="utf-8").to_records(
    index=False
)
test_triplets = pd.read_csv(f"./test_triplets.csv", encoding="utf-8").to_records(
    index=False
)
...
test_dataobj = {
    ...
    "meta_data": {
        ...
        "user_no2id": {
            x: str(x + train_dataobj["meta_data"]["num_users"])
            for x in range(len(test_user_data))
        },
        "item_no2id": {x: str(x) for x in range(len(item_data))},
        "knowledge_no2id": {x: str(x) for x in all_knowledge},
    },
    "process_args": {"min_logs_per_student": 150},
}
