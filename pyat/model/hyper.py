...
class IdentityNet(torch.nn.Module):
    ...
    def __init__(self, base_net: BaseModel, **kwargs) -> None:
        ...
        self.base_params = torch.nn.ParameterDict(
            {k: (torch.randn_like(v)) for k, v in base_params.items()}
        )
        self.identity = torch.nn.Identity()

    def forward(self) -> typing.Dict[str, torch.Tensor]:
        return {k: self.identity(v) for k, v in self.base_params.items()}
...
class NormalVariationalNet(torch.nn.Module):
    ...
    def __init__(self, base_net: BaseModel, **kwargs) -> None:
        ...
        self.log_std = torch.nn.ParameterDict(
            {k: (torch.rand_like(v) - 4) for k, v in base_params.items()}
        )
    ...
    def forward(self) -> typing.Dict[str, torch.Tensor]:
        ...
        for k in self.mean:
            ...
            out[k] = self.mean[k] + torch.randn_like(m, device=m.device) * torch.exp(
                input=log_s
            )
        return out
