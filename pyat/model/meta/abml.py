...
class AmortizedBayesianMetaLearning(MetaModel):
    def __init__(self, base_model: BaseModel, cfg: dict) -> None:
        ...
        assert not (
            self.no_bayes and self.fix_prior
        ), "Cannot remove Bayes and fix prior at the same time"
        if self.no_bayes:
            self.hyper_model = IdentityNet(base_model)
        if self.fix_prior:
            self.hyper_model = StandardNormalNet(base_model)

        self.adaptive_hyper_model: typing.Optional[
            StateDictType
        ] = None  # used for adaptive testing
        ...
        if self.inner_learn_lr:
            ...
            self.inner_lrs = torch.Tensor(
                [self.cfg["inner_lr"]] * self.inner_num_updates
            ).to(self.device)
    ...
    def adaptation(
        ...
        def inner_update(item_nos_input, labels_input):
            ...
            for _ in range(self.num_samples):
                fast_weights = functional_call(
                    self.hyper_model, adapted_hyper_model, args=()
                )
                preds = functional_call(
                    self.base_model, fast_weights, args=(item_nos_input,)
                )
    ...
    def prediction(
        self, item_nos: torch.LongTensor, adapted_hyper_model: StateDictType
    ) -> torch.Tensor:
        # generate task-specific parameter
        base_model_params = functional_call(
            self.hyper_model, adapted_hyper_model, args=()
        )
    ...
    def validation(
        ...
        sampled_preds = [
            self.prediction(item_nos=item_nos, adapted_hyper_model=adapted_hyper_model)
            for _ in range(self.num_samples)
        ]
        loss = (
            sum([nn.BCELoss()(preds, labels) for preds in sampled_preds])
            / self.num_samples
        )
    ...
    def get_user_params(self) -> StateDictType:
        with torch.no_grad():
            return functional_call(
                self.hyper_model,
                self.adaptive_hyper_model
                if self.adaptive_hyper_model is not None
                else {},
                args=(),
            )
    ...
    def get_meta_params(self) -> typing.Dict:
        if self.fix_prior:
            ...
            return {
                k: v for k, v in self.named_parameters() if k.startswith("hyper_model")
            }
    ...
    def update_user_params(self, session: typing.Dict, **kwargs) -> None:
        ...
        labels = (
            torch.tensor([session["all_logs"][i] for i in item_nos])
            .float()
            .to(self.device)
        )
        item_nos = torch.tensor(item_nos).long().to(self.device)
        self.adaptive_hyper_model = self.adaptation(
            item_nos, labels, adapted_hyper_model=self.adaptive_hyper_model
        )
    ...
    def forward(self, item_nos: torch.LongTensor):
        return (
            sum(
                [
                    self.prediction(
                        item_nos, adapted_hyper_model=self.adaptive_hyper_model
                    )
                    for _ in range(self.num_samples)
                ]
            )
