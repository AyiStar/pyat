...
class ModelAgnosticMetaLearning(MetaModel):
    def __init__(self, base_model: BaseModel, cfg: dict) -> None:
        ...
        self._adaptive_hyper_model: typing.Optional[
            StateDictType
    ...
    def validation(
        ...
        preds = self.prediction(
            item_nos=item_nos, adapted_hyper_model=adapted_hyper_model
        )
        loss = nn.BCELoss()(preds, labels)
    ...
    def get_user_params(self) -> StateDictType:
        with torch.no_grad():
            return functional_call(
                self.hyper_model,
                self._adaptive_hyper_model
                if self._adaptive_hyper_model is not None
                else {},
                args=(),
            )
    ...
    def update_user_params(self, session: typing.Dict, **kwargs) -> None:
        ...
        self._adaptive_hyper_model = self.adaptation(
            item_nos, labels, adapted_hyper_model=self._adaptive_hyper_model
        )

    def forward(self, item_nos: torch.LongTensor):
        base_model_params = functional_call(
            self.hyper_model, self._adaptive_hyper_model, args=()
        )
