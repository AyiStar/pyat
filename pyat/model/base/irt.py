...
class ItemResponseTheoryModel(BaseModel, nn.Module):
    ...
    def update_user_params(self, session):
        if (
            self.update_policy is not None
            and self.update_lr is not None
            and self.update_max_loop is not None
        ):
    ...
    def _update_user_params_optim(self, session):
        ...
        new_user_param = 0.0
        try:
            new_user_param = brentq(
                lambda x: self._deriv_likelihood(x, item_nos, labels),
                self._min_user_param,
                self._max_user_param,
            )
        except ValueError:
            if all(label == 1 for label in labels):
                ...
                global_logger.logger.debug(
                    f"May have same sign: f(a)={f_a:.4f}, f(b)={f_b:.4f}"
                )
                if f_a > 0 and f_b > 0:
                    new_user_param = self._max_user_param
                    ...
                    global_logger.logger.warn(
                        f"Some error occurs during updating user param, use average user param"
                    )
    ...
    def _deriv_likelihood(self, user_param, item_nos, labels):
        ...
        return sum(
            y - (1.0 / (1.0 + np.exp(-(user_param - item_param))))
            for y, item_param in zip(labels, item_params)
        )
