...
class BaseTrainer:
    ...
    def fit(self, train_data: Dict, val_data: Optional[Dict], cfg: DictConfig):
        ...
        self.user_params = {k: nn.Parameter(torch.zeros(train_dataset.num_users, *p.detach().shape).to(self.device),
                                            requires_grad=True)
                            for k, p in self.model.get_user_params().items()}
        for k, p in self.user_params.items():
            nn.init.xavier_normal_(p)
        train_dataloader = data.DataLoader(train_dataset, batch_size=max(1, batch_size // 16),
                                           collate_fn=session_dataset_collate_fn, shuffle=True, num_workers=0)
        loss_func = nn.BCELoss()
        optimizer = optim.Adam(list(self.user_params.values())
                               + list(self.model.get_item_params().values()), lr=lr)
        ...
        for ep in range(1, n_epochs + 1):
            ...
            glogger.logger.exp(f'[train] Epoch {ep}: acc={ep_acc:.4f}, auc={ep_auc:.4f}, loss={ep_loss:.4f} ')
    ...
    def _validation(self, val_dataset: SessionDataset):
        val_dataloader = data.DataLoader(val_dataset, batch_size=1,
                                         shuffle=False, collate_fn=session_dataset_collate_fn)
        all_preds = []
        all_labels = []
        self.model.eval()
        with torch.no_grad():
            for sessions in tqdm(val_dataloader, disable=self.silent):
                ...
                preds = functional_call(self.model, {k: p[user_no] for k, p in self.user_params.items()},
                                        args=(item_nos,))
        ...
        loss = nn.BCELoss()(torch.from_numpy(all_preds).double().to(self.device),
    ...
    def _generate_train_batch(self, sessions, sample_size=16) -> Tuple[List[int], List[torch.LongTensor], torch.FloatTensor]:
        user_no_list, item_nos_list, labels = [], [], []
        for sess in sessions:
            user_no_list.append(sess['user_no'])
            samples = random.sample(list(sess['all_logs'].keys()), sample_size) \
                if sample_size < len(sess['all_logs']) else list(sess['all_logs'].keys())
