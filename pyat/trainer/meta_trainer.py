...
class MetaTrainer:
    ...
    def fit(
        self,
        train_data: typing.Dict,
        val_data: typing.Optional[typing.Dict],
        test_data: typing.Optional[typing.Dict],
        cfg,
    ):
        ...
        for ep in range(1, n_epochs + 1):
            ...
            for batch in tqdm(train_dataloader, disable=self.silent):
                ...
                for session in sessions:
                    (
                        ...
                        preds,
                        labels,
                    ) = self._adapt_and_evaluate_on_session(
            ...
            glogger.logger.exp(
                f"[train] Epoch {ep}: acc={ep_acc:.4f}, auc={ep_auc:.4f}, meta loss={ep_meta_loss:.4f}"
            )
            ...
            if (
                ...
                and (self.best_model_snapshot is not None)
                and (ep % test_freq == 0)
            ):
                ...
                test_evaluator = CognitiveDiagnosisEvaluator(
                    best_model, device=self.device
                )
                for length in [3, 5, 10, 1000]:
                    ...
                    acc, auc, loss = [
                        sum(d[k] for d in test_results) / len(test_results)
                        for k in ("acc", "auc", "loss")
                    ]
                    glogger.logger.exp(
                        f"[test]Epoch {ep}: acc={acc:.4f}, "
                        f"auc={auc:.4f}, "
                        f"loss={loss:.4f}"
                    )
    ...
    def _adapt_and_evaluate_on_session(
        self, session: typing.Dict, test_length: int, each_step: bool
    ) -> typing.Tuple[
        typing.Dict[str, torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor
    ]:
        def _adapt_current_session(adapted_hyper_model_):
            # compute model output and loss
            item_nos = (
                session["selected"][-1:]
                if self.model.update_policy == "last"
            ...
            return self.model.adaptation(
                item_nos, labels, adapted_hyper_model=adapted_hyper_model_
            )
        ...
        meta_loss, preds = self.model.validation(
            val_item_nos, val_labels, adapted_hyper_model, return_prediction=True
        )
