    ...
    def _score_batch(self, item_nos: List, session) -> List:
        ...
        with torch.no_grad():
            if isinstance(model, AmortizedBayesianMetaLearning):
                sampled_predictions = [
                    model.prediction(
                        item_nos, adapted_hyper_model=model.adaptive_hyper_model
                    )
                    for _ in range(self.num_samples)
                ]
                item_preds = torch.transpose(
                    torch.stack(sampled_predictions), dim0=0, dim1=1
                )
                item_preds = (
                    torch.stack([item_preds, 1 - item_preds], dim=2).cpu().numpy()
                )
                # item_preds: [num_items, num_samples, 2]
                expected_entropy = np.mean(
                    -np.sum(xlogy(item_preds, item_preds), axis=-1), axis=1
                )
