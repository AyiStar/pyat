...
class MAATStrategy(BaseStrategy):
    ...
    def __init__(self, cfg: Dict):
        ...
        for knowledge_id in self.knowledge_weights:
            self.knowledge_weights[knowledge_id] = float(self.knowledge_weights[knowledge_id]) / float(norm_sum)
    ...
    def select_item(self, session: Dict) -> int:
        ...
        informativeness_candidates = [unselected[i] for i in np.argsort(informativeness_scores)][-self.k:]
        selected_knowledge = set()
        for i in session['selected']:
            selected_knowledge = selected_knowledge.union(set(self.item_data[i]['knowledge']))
        diversity_scores = []
        for item_no in informativeness_candidates:
            knowledge_ids = self.item_data[item_no]['knowledge']
            diversity_scores.append(sum([self.knowledge_weights[k] for k in knowledge_ids if k not in selected_knowledge]))
    ...
    def _expected_model_change_batch(self, item_nos: List[int], session: Dict):
        ...
        expected_grads_norm = preds * np.linalg.norm(pos_grads, axis=1) + (1 - preds) * np.linalg.norm(neg_grads, axis=1)
    ...
    def _expected_model_change(item_no, session):
        ...
        if isinstance(model, ItemResponseTheoryModel):
            ...
            expected_model_change_pos = abs(model.user_params.item() - tmp_model.user_params.item())
            ...
            del tmp_model
            expected_model_change = pred * expected_model_change_pos + (1 - pred) * expected_model_change_neg
