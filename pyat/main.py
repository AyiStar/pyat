...
def show_dataset_info(
    dataset_name: str, train_dataobj: Dict, test_dataobj: Dict, logger: logging.Logger
):
    ...
    info_table.add_column(
        ...
        [
            ...
            sum(list(map(len, train_dataobj["user_data"])))
            / len(train_dataobj["user_data"]),
    ...
    info_table.add_column(
        ...
        [
            ...
            sum(list(map(len, test_dataobj["user_data"])))
            / len(test_dataobj["user_data"]),
...
def main(cfg: DictConfig):
    ...
    if cfg.output_dir:
        cfg.output_dir = os.path.join(
            cfg.output_dir,
            f'EXP@{datetime.now().strftime("%Y%m%d-%H-%M-%S")}@{"_".join(exp_tags)}',
        )
    ...
    train_data_path = os.path.join(
        dataset_dir, dataset_name, f"{dataset_name}_train_seed_{dataset_seed}.pkl"
    )
    ...
    test_data_path = os.path.join(
        dataset_dir, dataset_name, f"{dataset_name}_test_seed_{dataset_seed}.pkl"
    )
    ...
    assert (load_model_dir is None) or (
        os.path.exists(load_model_dir)
    ), f"Invalid load_model_dir: {load_model_dir}"
    with open_dict(cfg):
        cfg.device = device
    ...
    if use_meta_model:
        model: MetaModel = build_meta_model(
            meta_model_cfg.class_name, model, meta_model_cfg
        )
    model.to(device)
    ...
    if evaluator_cfg is not None:
        ...
        if evaluator_cfg.name == "at":
            ...
            stopping_criterion = build_stopping_criterion(
                stopping_criterion_cfg.class_name, stopping_criterion_cfg
            )
            evaluator = AdaptiveTestingEvaluator(
                model, strategy, stopping_criterion, device=device
            )
        else:
        ...
        # save test result
        with open(
            os.path.join(cfg.output_dir, f'test_result@{"_".join(exp_tags)}.txt'), "w"
        ) as f:
            if evaluator_cfg.name == "at":
                for i in sorted(test_result.keys()):
                    f.write(
                        f'Test step {i}: acc={test_result[i]["acc"]:.4f}, auc={test_result[i]["auc"]:.4f}, '
                        f'rmse={test_result[i]["rmse"]:.4f}\n'
                    )
                    global_logger.logger.exp(
                        f'Test step {i}: acc={test_result[i]["acc"]:.4f}, '
                        f'auc={test_result[i]["auc"]:.4f}, rmse={test_result[i]["rmse"]:.4f}'
                    )
            else:
                f.write(
                    f'acc={test_result["acc"]:.4f}, auc={test_result["auc"]:.4f}, rmse={test_result["rmse"]:.4f}\n'
                )
                global_logger.logger.exp(
                    f'acc={test_result["acc"]:.4f}, auc={test_result["auc"]:.4f}, rmse={test_result["rmse"]:.4f}'
                )
        ...
        if "log" in test_result:
            torch.save(
                test_result["log"],
                os.path.join(cfg.output_dir, f'test_log@{"_".join(exp_tags)}.pt'),
            )

        # save cfg
        torch.save(cfg, os.path.join(cfg.output_dir, f'cfg@{"_".join(exp_tags)}.pt'))
        with open(
            os.path.join(cfg.output_dir, f'cfg@{"_".join(exp_tags)}.txt'), "w"
        ) as f:
